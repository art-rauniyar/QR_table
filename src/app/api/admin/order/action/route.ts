...
export async function POST(req: Request) {
	try {
		...
		if (body.action == 'accept') {
			...
			if (!order.acceptedAt) {
				order.acceptedAt = new Date() as any;
				console.log('✅ Setting acceptedAt:', order.acceptedAt);
			}
		}
		...
		if (body.action == 'complete') {
			...
			order.completedAt = new Date() as any;
			console.log('✅ Setting completedAt:', order.completedAt);
		}

		await order.save();
		console.log('✅ Order saved with timestamps:', { acceptedAt: order.acceptedAt, completedAt: order.completedAt });
